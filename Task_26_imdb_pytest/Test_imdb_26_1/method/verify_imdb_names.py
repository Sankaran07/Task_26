...
class Advance_search_fields(Element):
    ...
    def click_signin_popup(self):
        pop_up =self.driver.find_element(by=By.XPATH, value=self.sign_value)
        assert self.driver.find_element(by=By.LINK_TEXT, value='Sign In').is_displayed()
        sign_in_close = self.driver.find_element(by=By.XPATH, value=self.sign_close_icon)
    ...
    def click_and_enter_value_name_field(self, name_text):
        try:
            ...
            name_drop_down =wait.until(ec.element_to_be_clickable(self.dropdown_name))
